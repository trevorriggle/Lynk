...
function Section({ title, defaultOpen = true, children }) {
  ...
  return (
    <div className="mb-3">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-3 py-2 rounded-xl bg-teal-700 text-white shadow-sm"
      >
...
function Row({ label, onClick, muted = false }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left px-3 py-2 rounded-lg hover:bg-teal-50 active:bg-teal-100 border border-transparent hover:border-teal-200 ${muted ? "text-slate-500 italic" : "text-slate-800"}`}
    >
      {label}
    </button>
  );
...
export default function LeftStack({ onActivate }) {
  return (
    <aside className="h-full w-full lg:w-64 p-3 bg-teal-100/60">
      <Section title="Context Files">
        {/* example items from the mock */}
        <Row
      ...
      <Section title="Commands">
        {/* keyword-triggered functions you listed (e.g., flinn?, carolina?, fort-rapids?) */}
        <Row
          label="Carolina?"
