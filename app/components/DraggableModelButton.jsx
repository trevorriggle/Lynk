...
/** Claude + OpenAI with explicit endpoints */
const MODELS = [
...
const STORAGE_KEY = "lynk_pill_pos_v8"; // bump key to avoid old position collisions
...
export default function DraggableModelButton({ model, setModel }) {
  ...
  // Load/save pos
  useEffect(() => {
    try { const saved = localStorage.getItem(STORAGE_KEY); if (saved) setPos(JSON.parse(saved)); } catch {}
  }, []);
  useEffect(() => {
  ...
  useEffect(() => {
    const onDocDown = (e) => { if (!wrapRef.current) return; if (!wrapRef.current.contains(e.target)) setOpen(false); };
  ...
  const currentProvider = (MODELS.find((m) => m.label === current)?.provider) || "anthropic";
  const logoSrc = currentProvider === "anthropic" ? "/anthropic-logo.png" : "/OpenAI-Logo.png";

  return (
    <div ref={wrapRef} className="fixed z-[10001] select-none" style={{ left: pos.x, top: pos.y }}>
      {/* Pill */}
      <div
        role="button" aria-haspopup="listbox" aria-expanded={open} tabIndex={0}
      ...
      >
        <span className="flex min-w-0 items-center gap-2">
          <span className="flex items-center justify-center rounded-full bg-white flex-none" style={{ width: 36, height: 36 }}>
            <Image src={logoSrc} alt={currentProvider} width={28} height={28} className="object-contain" priority />
          </span>
          <span className="text-base leading-tight font-heading font-semibold tracking-normal truncate">{current}</span>
        </span>
        <span className={`transition-transform select-none flex-none ${open ? "rotate-90" : ""}`} aria-hidden>▸</span>
      </div>
      ...
      {open && (
        <div role="listbox" className="mt-2 w-[200px] rounded-2xl overflow-hidden shadow-xl !bg-[#176a82]">
          {MODELS.map((m, i) => (
            <button
              role="option" aria-selected={m.label === current} key={m.label} type="button"
              onClick={() => { setCurrent(m.label); setOpen(false); }}
              className="block w-full text-left px-4 py-2 text-white text-sm hover:bg-white/10"
              style={{ fontWeight: m.label === current ? 800 : 500, borderBottom: i < MODELS.length - 1 ? "1px dotted rgba(201,238,237,0.85)" : "none" }}
            >
              {m.label}
            </button>
          ))}
