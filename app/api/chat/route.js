...
export async function POST(req) {
  try {
    ...
    if (wantsAnthropic(model, provider)) {
      ...
      // Try requested model first, then safe fallbacks
      const candidates = [];
      ...
      for (const m of candidates) {
        ...
        // If auth/permissions error, stop trying fallbacks and show the exact provider response
        if (r.status === 401 || r.status === 403) {
          return new Response(`Claude ${r.status}: ${txt}`, { status: 502 });
        }
        // Otherwise try next candidate; if this was the last, return provider's message
